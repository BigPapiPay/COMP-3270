...
public class Autocomplete {
   ...
   /**
    ...
    public static class TrieAutocomplete implements Autocompletor {
         ...
         */
         //1st of 3 methods you must code
        private void add(String word, double weight) {
            //Exception Check: NullPointerException 
            if (word == null)
            {
               throw new NullPointerException("Word is an invalid input.");
            }
            //Exception Check: IllegalArgumentException
            if (weight < 0) 
            {
            ...
            //Variables
            char indicator;
            Node location = myRoot;
            for (int a = 0; a < word.length(); a++)
            {
               indicator = word.charAt(a);
               Node next = location.getChild(indicator);
               //Check to see if child node of the current location is null
               if (next == null) 
               {
                  next = new Node(indicator, location, weight); //Creates the new child node
                  location.children.put(indicator, next); // Adds the new child node to the tree
               }
               //Check that the max weight is the max weight
               if (weight > location.mySubtreeMaxWeight)
               {
                  location.mySubtreeMaxWeight = weight;
               }
               location = next;
            }
            //Set values for the added node
         ...
         *             NullPointerException if prefix is null
         */
         //2nd of 3 methods you must code
        public Iterable<String> topMatches(String prefix, int k) {
            //Exception Check: NullPointerException 
            if (prefix == null)
            {
               throw new NullPointerException("Prefix provided is invalid");
            }           
            //Variables
            char indicator;
            Node location = myRoot;
            ...
            //Iterables
            ArrayList<Node> wordAList = new ArrayList<Node>();
            ArrayList<String> finalList = new ArrayList<String>(); //Final Output
            ArrayList<String> output3 = new ArrayList<String>(); // Output for if there are no such words
            PriorityQueue<Node> nodePQ = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator());
            
            //Search
            for (int a = 0; a < prefix.length(); a++)
            {
               indicator = prefix.charAt(a);
               location = location.getChild(indicator);
               //IF the child is null, then return the emptystring output
               if (location == null) 
               {
                  return output3;
               }
            }
            //Adds the words into the PriorityQueue
            if (nodePQ != null)
            {
               nodePQ.add(location);
            }
            ...
            while (!nodePQ.isEmpty())
            {
               if (wordAList.size() >= k)
               {
                  // Figured out how to do the Collections.sort() from GeeksForGeeks
                  //https://www.geeksforgeeks.org/collections-reverseorder-java-examples/
                  Collections.sort(wordAList, Collections.reverseOrder());
                  break;
               }
               location = nodePQ.remove();
               if (location.isWord)
               {
                  wordAList.add(location);
               }
               for (Node no : location.children.values())
               {
                  nodePQ.add(no);
               }
            }
            //Used to move all words into the final output
            //Figured out how to implement Math.min() from GeeksForGeeks
            //https://www.geeksforgeeks.org/java-math-min-method-examples/
            for (int b = 0; b < Math.min(k,wordAList.size()); b++)
            {
               finalList.add(wordAList.get(b).myWord);
            }
            return finalList;            
         }
         ...
         *             NullPointerException if the prefix is null
         */
         // 3rd of 3 methods you must code
        public String topMatch(String prefix) {
            //Exception Check: NullPointerException 
            if (prefix == null)
            {
            ...
            {
               indicator = prefix.charAt(a);
               //Checks to see if you can continue
               //Had to use resources to understand how to implement this
               //https://www.geeksforgeeks.org/treemap-containskey-method-in-java/
               if (location.children.containsKey(indicator))
               {
                  location = location.getChild(indicator);
               }
               else 
            ...
            //Returns the empty string if no word with the largest weight does not exist
            if (match == false)
            {
               return emptyString;
            }
            while (location.mySubtreeMaxWeight > location.getWeight() && !location.isWord)
            {
               for (Node no : location.children.values()) 
               {
                  nodePQ.add(no);
            ...
            bestMatchFound = location.getWord();
        ...
        /**
         * Optional Bonus: Returns the highest weighted matches within k edit distance of
