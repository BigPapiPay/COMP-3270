...
public class AutocompleteTestGithub {
        /**
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         for (int i = 0; i < word.length(); i++) {
            ...
            index = word.charAt(i);
            nextNode = curr.getChild(index);
                
            if (nextNode == null) { // must create new node if necessary
               nextNode = new Node(index, curr, weight);
               curr.children.put(index, nextNode); // add to map
            }
            
            if (curr.mySubtreeMaxWeight < weight) {
               curr.mySubtreeMaxWeight = weight; // update new max weight if necessary
            }
            
            curr = nextNode; // point to next node and repeat
         }
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         //comparator allows queue to act as a max heap
         ...
         ArrayList<String> topMatchesString = new ArrayList<String>();
      
         if (k == 0) {
            return emptyList; // return list with 0 elements if k = 0
         }
         ...
         for (int i = 0; i < prefix.length(); i++) {
            ...
            if (curr == null) { // getChild returns null if no match is found
               return emptyList;
            }
         }
         
         if (curr != null) {
            nodeQueue.add(curr); // add to queue
         }
      
         do {  
            curr = nodeQueue.remove(); // remove head of sorted queue
         
            for (Node n : curr.children.values()) {
               nodeQueue.add(n); // add all children to queue 
            }
            
            if (topMatches.size() >= k) {
               // descending sort using Collection comparator--highest value at front of queue
         ...
         } while (!nodeQueue.isEmpty()); // terminate when nothing left in queue--all matches have been added
         
         for (Node n : topMatches) {
            topMatchesString.add(n.myWord);
         }
      
         return topMatchesString;
      }
      ...
      public String topMatch(String prefix) {
         ...
         for (int i = 0; i < prefix.length(); i++) {
            ...
            if (curr.children.containsKey(index)) { // if match, continue
               curr = curr.getChild(index);
            }
            else { // if no match, set to false and break
               wordExists = false;
               break;
            }
         }
         ...
         if (!wordExists) {
            return empty; // must return empty string if no prefix matches exist
         }
         
         while (!curr.isWord && curr.getWeight() < curr.mySubtreeMaxWeight) { // continue until max weight is reached
         
            for (Node n : curr.children.values()) {
               nodeQueue.add(n);
            }
            
            curr = nodeQueue.remove(); // retrieve head of queue
         }
         
         topMatch = curr.getWord();
      ...
      public double weightOf(String term) {
         ...
         for (int i = 0; i < term.length(); i++) { // traverse the trie 
            char index = term.charAt(i);
            curr = curr.children.get(index);
         }
         
         weight = curr.myWeight;
      
         if (!curr.isWord) {
            return notFound; // if term is not in dictionary, exit with 0
         }
         else {
            return weight;
         }
